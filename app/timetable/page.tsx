...
export default function Home() {
  ...
  useEffect(() => {
    supabase
      ...
      .order("name")
      .then(({ data }) => {
        if (data) setTeacherNames(data.map((t) => t.name));
      });
  }, []);
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 text-white p-3 md:p-6">
      <div className="mx-auto max-w-7xl">
        <div className="mb-4">
          <a href="/" className="text-sm text-violet-400 hover:text-violet-300">&larr; Home</a>
        </div>
        ...
        <aside className="bg-zinc-900 rounded-2xl p-5 space-y-4 h-fit">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              {form.instructor &&
                !teacherNames.some(
                  (n) => n.toUpperCase() === form.instructor,
                ) && (
                  <option value={form.instructor}>
                    {form.instructor} (not in DB)
                  </option>
                )}
            </select>
            <select
